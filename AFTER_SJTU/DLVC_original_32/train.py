...
if __name__ == '__main__':
    ...
    netG.to(device)
    # 用预训练好的qp=38对网络进行初始化
    netG.load_state_dict(torch.load('/home/li/Desktop/LZH/CopyCNN/DLVCQP32/yuxunlian38/DLVC_epoch_1_160.pth'))
    # netD.to(device)
    ...
    for epoch in range(start_epoch, NUM_EPOCHS + 1):
        ...
        torch.save(netG.state_dict(), 'epochs/DLVC_epoch_%d_%d.pth' % (UPSCALE_FACTOR, epoch))# model change lzh
        checkpoint = {
            "net": netG.state_dict(),
        ...
        if not os.path.isdir("./Checkpoint"):
            os.mkdir("./Checkpoint")
        torch.save(checkpoint, './Checkpoint/DLVC_best_%s.pth' % (str(epoch)))# model change lzh
