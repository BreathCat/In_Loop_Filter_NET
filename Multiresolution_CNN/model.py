...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.scale1_group2 = torch.nn.Sequential(
            ...
            self.make_layer(Group2, num_blocks=4, stride=1)

        )

        self.group2 = self.make_layer(Group2, num_blocks=5, stride=1)
...
def normal_init(m, mean, std):
    if isinstance(m, nn.ConvTranspose2d) or isinstance(m, nn.Conv2d):
        ...
        # m.bias.data.zero_()
        print('Multiresolution_CNN 每一层加上BatchNorm2d 正在初始化权重')
        # nn.init.xavier_uniform(m.weight)
